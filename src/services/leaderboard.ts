...
export const calculateLeaderboard = async (qualifierId: number, content: DataArray) => {
...
export const updateLeaderboard = async (qualifierId: number) => {
    ...
    if (qualifier === null) throw new Error(`No Qualifier with id ${qualifierId}.`);
    if (qualifier.cup.leaderboard === null) throw new Error("Cup has no leaderboard! YEK?");
